...
static constexpr std::size_t ARG_COUNT = 4;
...
int main(const int argc, const char ** argv)
{
    ...
    for (std::size_t generation = 0; generation < iterations; 
        ++generation, progress_bar(generation, iterations)) // always print progress
    {
...
{
    for (std::size_t x = 2; x < current.size() - 2; ++x)
        for (std::size_t y = 2; y < current.size() - 2; ++y)
        {
            // get hood
...
{
    std::cout << "\n\n"
        << "Would you like to create images ['y' or 'n'] > ";
    std::string input;
    std::getline(std::cin, input); // avoid overflow
    return std::tolower(input[0]) == 'y';
}
